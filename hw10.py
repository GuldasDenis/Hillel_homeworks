...
filtered_cities = filter(filter_cities, cities)
...
sorted(grade, key=lambda x: x[1], reverse=True)
...
rating_movies = lambda movie: float(movie["rating"]) > 8.5
filter_movies = list(filter(rating_movies, title_rating))

sorted_movies = sorted(filter_movies, key=lambda x: x['title'])
print(sorted_movies)
...
link = get_trailer_link(movie_title)
print(link)


def get_info(name):
  result = {'name': name, 'num_of_films': 0, 'movies': []}
  for film in imdb_rank:
    if name in str(film['writers']) or name == film['director']:
      result['num_of_films'] += 1
      result['movies'].append({'title': film['title'], 'rating': film['rating']})
  return result

name = input("Введіть ім'я  режисера/сценариста: ")
...
for film in imdb_rank:
    for i in film['genre']:
        title_genre.append(i)
title_genre = set(title_genre)


def film_search(name):
    films = []
    for i in imdb_rank:
...
name = input("Введіть жанр для пошуку фільмів: ")
print(film_search(name))